...
from gluonts.torch.distributions.distribution_output import (
    DistributionOutput, #父类DistributionOutput，之后的全连接pdf输出层，都是该DistributionOutput的子类
    LambdaLayer,
    PtArgProj,
)
...
class NormalOutput(IndependentDistributionOutput):
    ...
    def __init__(self, dim: Optional[int] = None) -> None:
        super().__init__(dim)
        if dim is not None: #将分布参数维度，从默认的1维，改成dim维
            self.args_dim = {k: dim for k in self.args_dim}

    @classmethod
    def domain_map(cls, loc, scale):
        scale = F.softplus(scale) #标准差σ----全连接层的线性函数+softplus激活函数
...
class NormalMixtureOutput(DistributionOutput):
    ...
    def __init__(self, components: int = 1) -> None:
        self.components = components
        self.args_dim = { #分布参数维度 = 混合分布个数components
            "mix_logits": components,
            "loc": components,
            "scale": components,
        }
    ...
    def domain_map(cls, mix_logits, loc, scale):
        scale = F.softplus(scale) #标准差σ----全连接层的线性函数+softplus激活函数
        return mix_logits.squeeze(-1), loc.squeeze(-1), scale.squeeze(-1) #squeeze最后一维如果维数是1，就去掉最后一维（比如100*2*1变成100*2)

    def distribution(  ##输入分布参数distr_args，输出概率分布distr
        self, distr_args, scale: Optional[torch.Tensor] = None
    ) -> Distribution:
        ...
        if scale is None:
            ...
            return TransformedDistribution(distr, [AffineTransform(loc=0, scale=scale)]) #返回的是这个类型
