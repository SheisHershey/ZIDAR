...
读取数据集
'''
df = pd.read_csv('../ZIP-DeepAR代码/data/InterSim层次聚类后的Q料202001-202302(halfmonth).csv', header=0)
#df = pd.read_csv('../ZIP-DeepAR代码/data/carparts1.csv', header=0)
print(df)
...
'''
构造训练集、测试集（一共时间序列长度为76个半月，以2020-01-15~2022-12-30的前72个半月为训练集，以2023-01-15第73~74个半月为测试集）
'''
##提取test_y
test_y = []
for x in df.values:
...
estimator = DeepAREstimator(freq="1m",context_length=context_l,#需要根据代码报错来设定，input.size(-1) must be equal to input_size
                            prediction_length=1)
