...
from pts.model.deepar.tcn import TemporalConvNet #!! 时序卷积层
...
class DeepARTrainingNetwork(DeepARNetwork):
    def distribution(
        ...
        '''
        ！！消融实验
        # 生成与 mix_logits 形状相同的随机张量
        random_probs = torch.rand_like(mix_logits)
        # 归一化，使其总和为 1
        mix_logits = random_probs / random_probs.sum(dim=-1, keepdim=True)
        '''
            ...
            for j in range(context_start_index, sequence.shape[1]): #填充所有context_length + predict_length中的时间点(每个时间点t, 真实分位数qt填充xt)
                ...
                for i in range(sequence.shape[0]):
                    if sequence[i, j] == 0: # 若真实值为0，则进行缺失值填充
                        ...
                        #sequence_imput[i, j] = sequence[i, j] #消融实验
...
class DeepARPredictionNetwork(DeepARNetwork):
        ...
        for k in range(self.prediction_length):
            ...
            for i in range(new_samples.shape[0]):
                if new_samples[i, 0] <= 0:  # 若预测值<=0，进行缺失值填充
                        ...
                        #new_samples_imput[i, 0] = new_samples[i, 0]   #消融实验
    ...
    def forward(
        ...
        '''
        消融实验
        random_probs = torch.rand_like(mix_logits)# 生成与 mix_logits 形状相同的随机张量
        probs = random_probs / random_probs.sum(dim=-1, keepdim=True)# 归一化，使其总和为 1
        '''
